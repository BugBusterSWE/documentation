...
\subsection{\glossaryItem{Processo} di gestione}
...
\subsection{\glossaryItem{Processo} di infrastruttura}
...
È stato stabilito che ogni commit deve appartenere a un \glossaryItem{task}. I \glossaryItem{task}, assegnati dal \textit{responsabile} ai vari membri del gruppo corrispondono a \glossaryItem{issue} su \glossaryItem{GitHub}. All'apertura della \glossaryItem{issue} corrisponde alla creazione di un branch, il cui nome sarà conforme alla seguente nomenclatura: \textbf{issue\#\textit{numero issue}}. L'assegnatario del task, durante il suo svolgimento, viene coadiuvato da un sistema di \glossaryItem{Continuos Integration}, al termine dello svolgimento di tale task l'assegnatario esegue una \glossaryItem{Pull Request}, seguita da un controllo di contenuto da parte di un verificatore umano.

\paragraph*{Sistema di ticketing} I task, come già accennato, vengono assegnati tramite la piattaforma \glossaryItem{TeamWork}. All'assegnazione di un ticket su questa piattaforma corrisponde all'apertura di una \glossaryItem{issue} su \glossaryItem{GitHub} e ad una notifica tramite il sistema di messaggistica \glossaryItem{Slack}.

\paragraph*{Sistema di \glossaryItem{CI}}Il sistema di \glossaryItem{CI} controlla che il contenuto di ogni repository sia conforme alle \NormeDiProgetto. L'integrazione con \glossaryItem{GitHub} permette per ogni \glossaryItem{task} la verifica di ciascun commit. Attualmente, i sistemi di \glossaryItem{CI} utilizzati sono \glossaryItem{Travis} e \glossaryItem{Jenkins}.

\paragraph*{Sistema di Dashboard}Il responsabile utilizza una \glossaryItem{dashboard} creata con i seguenti strumenti:
\begin{itemize}

\item \glossaryItem{TeamWork} per l'assegnazione e la gestione dei \glossaryItem{task}
\item \glossaryItem{Chronos}, software sviluppato internamente al gruppo in grado di fornire al \textit{responsabile} una visione globale dell'andamento dei processi
\item \glossaryItem{Mephisto} progetto derivante dal \textit{fork} di \glossaryItem{Tracy}, software di tracciamento dei requisiti sviluppato da un gruppo di questo corso. Tale software è stato accuratamente modificato e riadattato alle nostre necessità.
\end{itemize}

\subsection{\glossaryItem{Processo} di miglioramento}
...
\subsection{\glossaryItem{Processo} di formazione}
