...
\item \textbf{Controlli dinamici}: Dopo una compilazione avvenuta con successo e dopo aver passato con successo i test di tipo statico vengono eseguiti i test di tipo dinamico tramite il programma \textit{mocha}, che si occupa di eseguire i test di unità e sengalare eventuali test falliti. È possibile generare un report di tipo \textit{xUnit}: in questa maniera per ogni volta che vengono eseguiti i test si può avere una storia e quindi una tracciabilità dell'andamento dei test. I test saranno scritti in Typescript e per ogni test devono seguire la seguente struttura:
\begin{verbatim}
