...
\section{Processi primari}
	...
	\subsection{Processo di sviluppo}
		...
		\paragraph*{3. Attività di Codifica}
                ...
                \item \textit{member-access}: questa regola impone l'utilizzo di dichiarare esplicitamente la visibilit\`a dei membri di una classe;
                \item \textit{member-ordening}: questa regola impone un ordinamento consistente dei membri delle classi, per fornire una lettura del codice pi\`u facile e maggiormente navigabile. In particolare, \`e stato deciso di scrivere prima le varibili e dopo i metodi, e per i medoti prima i metodi pubblici e dopo quelli privati;
                \item \textit{no-any}: questa regola impedisce la dichiarazione di variabile di tipo \textit{any} nel codice Typescript, eliminando di fatto la non determinazione di tipo a \textit{static time};
                ...
                \item \textit{curly}: questa regola impone l'utilizzo di parentesi grafe anche per \textit{statements} condizionali con istruzioni da una riga. Questo permette una maggiore leggibilit\`a del codice e contribuisce a manterlo ordinato e a prevenire errori;
                \item \textit{no-bitwise}: questa regola disabilita l'utilizzo delle cosidette operazioni \textit{bitwise} all'interno di espressioni. Questo in quanto queste operazioni sono solitamente fonte di errore e degradano la leggibilit\`a del codice, rendendolo pi\`u confusionario. Per questo \`e stato deciso di eliminare il loro utilizzo;
                ...
                \item \textit{no-duplicate-key}: questa regola impedisce la dichiarazioni di oggetti litterali con la stessa chiave;
                \item \textit{no-duplicate-variable}: questa regola impedisce la dichiarazioni di variabile con lo stesso nome nello stesso \textit{scope}, evitando ridichiarazioni del compilatore e quindi evitando errori identificabili a tempo di esecuzione;
                \item \textit{no-empty}: questa regola impedisce l'utilizzo di blocchi vuoti di codice, in quanto potrebbe indicare del codice mancante non implementato e potrebbe essere fonte di errore;
                \item \textit{no-eval}: questa regola impedisce l'uso della funzione \textit{eval()} in Typescript, in quando \`e ritenuta pericolosa dal punto di vista della sicurezza e pu\`o permettere l'esecuzione arbitraria di codice passato come una stringa;
                \item \textit{no-null-keyword}: questa regola impedisce l'uto della \textit{keyword} \textit{null} in favore dell'utilizzo della \textit{keyword} \textit{undefined}. Questo permette di uniformare il codice con l'utilizzo di una sola \textit{keyword};
                \item \textit{no-string-literal}: questa regola impedisce l'accesso ad oggetti tramite stringe litterali, incoraggiando l'utilizzo delle propriet\`a di accesso di tipo fortemente tipate;
                \item \textit{no-swith-case-fall-through}: questa regola impedisce l'utilizzo di \textit{switch} senza l'utilizzo della \textit{keyword} \textit{break} per ogni caso. In questa maniera vengono evitati errori in cui tutti i casi successivi a quello voluto vengano eseguiti involontariamente;
                \item \textit{no-unreachable}: questa regola impedisce la codifica di codice non raggiungibile in nessun caso durante l'esecuzione. In questa maniera \`e pi\`u facile riconoscere eventuali errori logici al programmatore;
                \item \textit{no-unused-expression}: questa regola impedisce la dichiarazione di oggetti senza un loro effettivo utilizzo. In questa maniera viene evitato che vengano dichiarati oggetti inutilizzati.
                ...
                \item \textit{trailing-comma}: questa regola impone nell'utilizzo di \textit{array} di oggetti litterali l'utilizzo della virgola;
                \item \textit{align}: questa regola impone che sia presente un allignamento uniforme tra le varie istruzioni del codice, per facilitarne una migliore lettura;
                \item \textit{class-name}: questa regola impone l'utilizzo della nomenclatura delle classi di tipo \textit{PascalCased};
                ...
                \item \textit{one-line}: questa regola impone che una singola istruzione sia presente su una riga e non venga mandata a capo. Questo per migliorare la lettura del codice;
                ...
                \item \textit{variable-name}: questa regola impone la dichiarazioni di variabili seguendo lo stile \textit{camelCase}. Inoltre viene anche vietato di dare nomi a variabili corrispondenti a eventuali \textit{keywords};
