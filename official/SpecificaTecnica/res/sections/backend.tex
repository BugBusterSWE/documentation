...
\paragraph*{Descrizione}
Package che racchiude le classi che rappresentano la \textit{business logic} dell'applicazione. Ciascuna di queste classi implementa la classe astratta Model dichiarata all'interno del \glossaryItem{package}.
...
\begin{itemize}
\item MongooseConenction, per la connessione con il database \glossaryItem{MongoDB} dell'applicazione \glossaryItem{MaaS};
...
\begin{itemize}
\item Mongoose, per l'interazione con il database \glossaryItem{MongoDB} dell'applicazione \glossaryItem{MaaS};
\item Mongoose-validator, per la \glossaryItem{validazione} degli schemi e dei dati memorizzati;
...
\item Mongoose, per l'interazione con il database \glossaryItem{MongoDB} dell'applicazione \glossaryItem{MaaS};
\item Mongoose-validator, per la \glossaryItem{validazione} degli schemi e dei dati memorizzati;
\item Router::CompanyRouter, per la gestione delle richieste dirette verso il modello delle \glossaryItem{Company};
\end{itemize}
...
\item Mongoose, per l'interazione con il database \glossaryItem{MongoDB} dell'applicazione \glossaryItem{MaaS};
\item Mongoose-validator, per la \glossaryItem{validazione} degli schemi e dei dati memorizzati;
\item Lib::DSLChecker, per il controllo di validità (sintattica e per la \glossaryItem{verifica} dei permessi di accesso alle Collections di un database) della specifica \glossaryItem{DSL};
\item Router::DSLRouter, per la gestione delle richieste dirette verso il modello delle specifiche \glossaryItem{DSL}.
\end{itemize}

\paragraph{MongooseConnection}
\paragraph*{Descrizione}

Si occupa della connessione al database \glossaryItem{MongoDB} dell'applicazione utilizzando Mongoose.

\paragraph*{Utilizzo}
Attraverso questa classe è possibile connettersi al database \glossaryItem{MongoDB} dell'applicazione e operare sulla connessione ad esso.

\paragraph*{Relazioni con altri \glossaryItem{moduli}}
\begin{itemize}
\item Mongoose, per la connessione al database \glossaryItem{MongoDB} dell'applicazione \glossaryItem{MaaS}.
\end{itemize}
...
\begin{itemize}
\item Lib::AuthenticationChecker, per il controllo dell'avvenuta autenticazione dell'utente;
\item Model::CompanyModel, per il soddisfacimento delle richieste verso il modello di dati delle \glossaryItem{Company}.
\end{itemize}
...
\item Model::DSLModel, per il soddisfacimento delle richieste verso il modello di dati delle specifiche \glossaryItem{DSL}.
\end{itemize}
...
\item DatabaseRouter
\end{itemize}
Essendo l'implementazione del design pattern Facade, RouterFacade espone un'interfaccia base per il \glossaryItem{package} Routes. I metodi esposti da RouterFacade conterranno chiamate a metodi specifici di UserRouter, CompanyRouter, DSLRouter e DatabaseRouter.
...
\paragraph*{Descrizione}
Classe astratta comune alle configurazioni usate da \glossaryItem{MaaS}.
...
\paragraph*{Relazione con altri \glossaryItem{moduli}}
\begin{itemize}
\item MongoDBConnection, per i parametri della connessione al database \glossaryItem{MongoDB};
...
\paragraph*{Utilizzo}
Viene utilizzata per scegliere la configurazione di \glossaryItem{MaaS} da adottare.
...
\paragraph*{Descrizione}
Classe utilizzata per memorizzare i parametri della connessione al database \glossaryItem{MongoDB}.
...
\paragraph*{Relazione con altri \glossaryItem{moduli}}
\begin{itemize}
\item Model::DSLModel, per la risposta sul controllo della validità di una specifica \glossaryItem{DSL}
\end{itemize}
...
\item Router::RouterFacade, per la gestione delle \glossaryItem{API} esposte dal server;
\item BodyParser, per il parsing del corpo della richiesta;
\item ExpressJS, per la creazione ed esposizione delle \glossaryItem{API} \glossaryItem{REST}.
\end{itemize}
...
\textbf{API:} /api/login \\
\textbf{Descrizione:} Necessita di una richiesta con \textit{body} contenente email e password dell'utente. Lo scopo della richiesta è quello di autenticare l'utente presso \glossaryItem{MaaS} e di rendergli disponibili funzionalità dipendenti dal suo ruolo e dai suoi permessi. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{Descrizione:} Metodo per la creazione di un utente invitato da una \glossaryItem{Company}. Necessita di una richiesta con \textit{body} contenete le informazioni per la creazione completa di un utente, in particolare indirizzo email, password e \glossaryItem{Company} di riferimento. A seguito della richiesta un nuovo utente è registrato presso \glossaryItem{MaaS} e collegato alla \glossaryItem{Company}. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{Descrizione:} Necessita di una richiesta con \textit{body} contenente le informazioni relative alla \glossaryItem{Company} e alla creazione del profilo del suo \glossaryItem{Owner}. Queste informazioni includono indirizzo email e password dell'Owner e nome della \glossaryItem{Company} da creare. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{Descrizione:} Necessita di una richiesta con \textit{body} contenente l'email e il livello di accesso dell'utente da creare, che viene memorizzato presso il database interno di \glossaryItem{MaaS}; i permessi di cui godrà sono dipendenti dal ruolo scelto, e possono essere modificati in qualsiasi momento da un Admin o da un \glossaryItem{Owner}. Per accertare che sia l'Owner ad inviare la richiesta viene controllato il livello dell'utente autenticato. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{Livello di accesso minimo:} OWNER \\
\textbf{Descrizione:} L'utente della \glossaryItem{Company} con ID :company\_id corrispondente all'utente con ID :user\_id viene rimosso dal database di \glossaryItem{MaaS} e scollegato dalla \glossaryItem{Company} di appartenza. A seguito della richiesta viene ritornato messaggio di conferma dell'avvenuta cancellazione. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{Livello di accesso minimo:} GUEST \\
\textbf{Descrizione:} Ritorna le informazioni generali della \glossaryItem{Company} corrispondente all'ID :company\_id. Se la \glossaryItem{Company} non esiste viene ritornato un messaggio di errore, altrimenti vengono ritornati il nome della \glossaryItem{Company} e l'indirizzo email del suo \glossaryItem{Owner}. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{Descrizione:} Necessita di una richiesta con \textit{body} contenente le modifiche da apportare ai dati della \glossaryItem{Company} corrispondente all'ID :company\_id. Se la \glossaryItem{Company} non esiste viene ritornato un messaggio di errore. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{API:} /api/companies/:company\_id/ \\
\textbf{Livello di accesso minimo:} OWNER \\
\textbf{Descrizione:} Rimuove la \glossaryItem{Company} corrispondente all'ID :company\_id. Se la \glossaryItem{Company} non esiste viene ritornato un messaggio di errore, altrimenti viene restituita una conferma di cancellazione. La cancellazione della \glossaryItem{Company} comporta anche la cancellazione di tutti gli utenti associati ad essa. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{Livello di accesso minimo:} GUEST \\
\textbf{Descrizione:} Ritorna un array contenente le specifiche \glossaryItem{DSL} alle quali l'utente ha accesso in formato \glossaryItem{JSON}. Questo array potrebbe essere vuoto, nel caso in cui l'utente non avesse accesso a nessuna specifica \glossaryItem{DSL}; in caso contrario contiene ogni informazione necessaria a distinguere le specifiche l'una dall'altra. A partire da questo elenco l'utente, dipendentemente da ruoli e permessi, può eseguire, leggere o modificare una specifica \glossaryItem{DSL}. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{API:} /api/companies/:company\_id/DSLs \\
\textbf{Livello di accesso minimo:} MEMBER \\
\textbf{Descrizione:} Necessita di una richiesta con \textit{body} contenente i dati necessari alla creazione della specifica \glossaryItem{DSL}. La specifica \glossaryItem{DSL} viene validata, in modo da assicurare che non acceda a \glossaryItem{Collection} non accedibili dagli utenti Member e che sia sintatticamente corretta. Se questi controlli vengono superati con successo la specifica \glossaryItem{DSL} viene salvata e associata alla \glossaryItem{Company}, con la restituzione di un messaggio di conferma, altrimenti viene ritornato un messaggio di errore. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{API:} /api/companies/:company\_id/DSLs/:dsl\_id \\
\textbf{Livello di accesso minimo:} MEMBER \\
\textbf{Descrizione:} Necessita di una richiesta con body contenente i dati necessari alla modifica della specifica \glossaryItem{DSL} identificata da dsl\_id. Tutti i controlli descritti per la creazione della specifica \glossaryItem{DSL} vengono ripetuti, in modo da assicurare che la modifica non abbia introdotto errori. Anche in questo caso, se i controlli vengono superati con successo la specifica \glossaryItem{DSL} viene salvata, con la restituzione di un messaggio di conferma, altrimenti viene ritornato un messaggio di errore. \\
\textbf{Scenario:}
\begin{figure}[H]
\centering
...
\textbf{API:} /api/companies/:company\_id/DSLs/:dsl\_id \\
\textbf{Livello di accesso minimo:} MEMBER \\
\textbf{Descrizione:} Ritorna un messaggio in formato \glossaryItem{JSON} di avvenuta cancellazione se esiste una specifica \glossaryItem{DSL} corrispondente a :dsl\_id, altrimenti viene ritornato un messaggio di errore. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{Livello di accesso minimo:} GUEST \\
\textbf{Descrizione:} Ritorna la \glossaryItem{Dashboard} di un utente definita da una specifica \glossaryItem{DSL}. Ovviamente potrebbe non esistere una specifica \glossaryItem{DSL} che descrive la \glossaryItem{Dashboard} di un utente: in questo caso viene restituita la \glossaryItem{Dashboard} di default contenente le specifiche \glossaryItem{DSL} accedibili dall'utente. \\
\textbf{Scenario:}  
\begin{figure}[H]
\centering
...
\textbf{Livello di accesso minimo:} GUEST \\
\textbf{Descrizione:} Ritorna un \glossaryItem{JSON} contenente i dati richiesti dalla specifica \glossaryItem{DSL} e la struttura su cui inserire i dati. In questo modo eventuali funzioni inserite nella specifica \glossaryItem{DSL} possono essere esguite lato client, evitando di bloccare l'esecuzione lato server nel caso di funzioni maligne inserite per un tentativo di attacco di tipo DoS (\textbf{D}enial \textbf{o}f \textbf{S}ervice. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\newpage
\paragraph{Invio di una Collection} \mbox{} \\
\textbf{Tipologia:} GET \\
\textbf{API:} /api/collection/:collection\_id \\
\textbf{Livello di accesso minimo:} GUEST \\
\textbf{Descrizione:} Invia per email la Collection selezionata. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{res/sections/backend/sequence/(GET)sendCollection.png}
\caption{Invio di una Collection}
\end{figure}

\newpage
\paragraph{Invio di una Document} \mbox{} \\
\textbf{Tipologia:} GET \\
\textbf{API:} /api/document/::document\_id \\
\textbf{Livello di accesso minimo:} GUEST \\
\textbf{Descrizione:} Invia per email il Document selezionato. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{res/sections/backend/sequence/(GET)sendDocument.png}
\caption{Invio di una Document}
\end{figure}
...
\textbf{Livello di accesso minimo:} MEMBER \\
\textbf{Descrizione:} Ritorna un array contenente i nomi e gli id di ciascun database. In questo modo un utente sa come riferirsi ad un database nella scrittura di una specifica \glossaryItem{DSL}. \\
\textbf{Scenario:}
\begin{figure}[H]
\centering
...
\textbf{API:} /api/companies/:company\_id/databases/:database\_id \\
\textbf{Livello di accesso minimo:} ADMIN \\
\textbf{Descrizione:} Elimina il database selezionato, tutte le specifiche \glossaryItem{DSL} che lo utilizzano e le Collections ad esso collegate. L'esecuzione di questa richiesta comporta un pericoloso effetto domino che può portare a distruggere i dati di una \glossaryItem{Company}. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{Livello di accesso minimo:} MEMBER \\
\textbf{Descrizione:} Ritorna un array di \glossaryItem{Collection} relative ad un database a cui l'utente ha accesso. L'array ritornato può essere vuoto: questo accade quando l'utente non ha accesso a nessuna \glossaryItem{Collection} di quel database. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{Descrizione:} Restituisce un array di \glossaryItem{JSON} contenenti le informazioni relative alle \glossaryItem{Company} presenti nell'applicazione, in particolare nome e indirizzo email dell'Owner. Se non sono presenti \glossaryItem{Company} l'array ritornato è vuoto. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{Descrizione:} Necessita di una richiesta con \textit{body} contenente l'indirizzo email e il ruolo dell'utente da creare, che verrà associato alla \glossaryItem{Company} individuata da \glossaryItem{company}\_id. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
...
\textbf{Tipologia:} GET \\
\textbf{API:} /api/admin/superadmins \\
\textbf{Descrizione:} Verranno ritornati tutti i SuperAdmin di MaaS. La richiesta non necessita di nessuna informazione nel body. \\
\textbf{Scenario:} 
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{res/sections/backend/sequence/(GET)superadminSA.png}
